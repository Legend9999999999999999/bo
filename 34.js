      ...
      let admin = await db.collection('admindb').find({ admin: "admin" }).toArray()
        ...
        if (bots == 'Active') {
            ...
            if (flag == channel.length) {
                ctx.replyWithMarkdown(
                    '*<b>🎁Giveaway For All Paytm Cash Users</b>\n\nhttps://Tg-a.xyz/C/ydQK\n<b>Claim Every User 💥💥</b>*', { reply_markup: { keyboard: [['💰 Balance'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['💳 Withdraw', '📊 Statistics']], resize_keyboard: true } }
                )
...
ent-Type': 'application/json',
         ...
         var response = "";
         var post_req = https.request(options, function (post_res) {
             post_res.on('data', function (chun